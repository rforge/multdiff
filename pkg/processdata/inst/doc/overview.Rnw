...
are equidistant and observed at time points \verb+0, 1, ..., 1999+. The
summary tells us that the data set contains 2000 observations (the
number of grid points in the observation grid) in the observation window from 0 to 1999. 
...
<<fig=TRUE, eps=FALSE, keep.source=TRUE>>=
p <- plot(processX) + facet_grid(.~.) + aes(colour = id)
...
For the interested, the current implementation relies on an internal
...
copies of the data set are made. Even subsetting do
